...
.. function:: get_smoothscale_backend
   ...
   Shows whether or not smoothscale is using ``MMX`` or ``SSE`` _ACCELERATION.
   If no _ACCELERATION is available then "GENERIC" is returned. For a x86
   processor the level of _ACCELERATION to use is determined at runtime.
...
.. function:: set_smoothscale_backend
   ...
   Sets smoothscale _ACCELERATION. Takes a string argument. A value of 'GENERIC'
   turns off _ACCELERATION. 'MMX' uses ``MMX`` instructions only. 'SSE' allows
